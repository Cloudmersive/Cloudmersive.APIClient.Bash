...
(Get-Content ./client/cloudmersive-cli) -replace "if [[ -z `"$host`" ]]; then*ERROR_MSG=`"ERROR: No hostname provided!!! You have to  provide on command line option '--host ...'`"*exit 1*fi", "" | Set-Content ./client/cloudmersive-cli
